...
const Quiz = ({navigation}) => {
  ...
  const getQuiz = async () => {
    ...
    try {
      const customQuestions = require('../json/Custom_questions.json');
      setQuestions(customQuestions.results);
      setOptions(generateOptionsAndShuffle(customQuestions.results[0]));
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      {isLoading ? <View style={{display:'flex', justifyContent:'center', alignItems:'center', height:'100%'}}>
        <Text style={{fontSize:32, fontWeight:'700',color:'blue'}}>LOADING...</Text>
      </View> : questions && (
